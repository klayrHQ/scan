...
const Page = async (props: any) => {
  ...
  if (event?.data.chainID) {
    const chain = await getData("lisk-service", "get.blockchain.apps.meta", {
      chainID: event.data.chainID,
    });
    event.data.chainName = chain?.data
      ? chain?.data[0]?.displayName
      : undefined;
  }

  if (event?.data.ccm?.receivingChainID) {
    let chain = await getData("lisk-service", "get.blockchain.apps.meta", {
      chainID: event.data.ccm?.receivingChainID,
    });
    if (chain?.data?.length <= 0) {
      chain = await getData("lisk-service", "get.blockchain.apps", {
        chainID: event.data.ccm?.receivingChainID,
      });
    }
    event.data.ccm.receivingChainName = chain?.data
      ? chain?.data[0]?.displayName || chain?.data[0]?.chainName
      : undefined;
  }

  if (event?.data.ccm?.sendingChainID) {
    let chain = await getData("lisk-service", "get.blockchain.apps.meta", {
      chainID: event.data.ccm?.sendingChainID,
    });
    if (chain?.data?.length <= 0) {
      chain = await getData("lisk-service", "get.blockchain.apps", {
        chainID: event.data.ccm?.sendingChainID,
    ...
    event.data.ccm.sendingChainName = chain?.data
      ? chain?.data[0]?.displayName || chain?.data[0]?.chainName
      : undefined;
  }
