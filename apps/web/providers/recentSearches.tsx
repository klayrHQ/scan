...
export const RecentSearchesProvider = ({ children }: { children: any }) => {
  const [recentSearches, setRecentSearches] = useState<
    Array<recentSearchesDataType> | undefined
  >();
  ...
  useEffect(() => {
    recentSearches &&
      localStorage.setItem("lastSearches", JSON.stringify(recentSearches));
  }, [recentSearches]);

  return (
    <RecentSearchesContext.Provider
      value={{ recentSearches, saveSearch, setRecentSearches }}
    >
      {children}
    </RecentSearchesContext.Provider>
  );
