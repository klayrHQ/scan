...
export const Validator = ({
  ...
  const kpis: {
    ...
    },
    {
      ...
      value:
        queryData["account-validator-id"]?.data[0]?.consecutiveMissedBlocks,
      type: "number",
      format: "number",
    },
    {
  ...
  const punishments =
    queryData["account-validator-id"]?.data[0]?.punishmentPeriods;
  ...
  return (
    <Grid columns={1} flex gap={4}>
      {punishments && punishments.length > 0 && (
        ...
        >
          <Typography
            className={"py-1 px-3 rounded-full w-max bg-warning text-onWarning"}
            tag={"span"}
          >
            {"Validator is being punished"}
          </Typography>
          {/*<Typography tag={"span"}>{"Details:"}</Typography>*/}
          <div className={"flex justify-between"}>
            <Typography tag={"span"}>{"Start height:"}</Typography>
            <Typography className={"font-bold"} tag={"span"}>
              {punishments[punishments.length - 1]?.start}
            </Typography>
          </div>
          <div className={"flex justify-between"}>
            <Typography tag={"span"}>{"End height:"}</Typography>
            <Typography className={"font-bold"} tag={"span"}>
              {punishments[punishments.length - 1]?.end}
            </Typography>
          </div>
        </Grid>
      )}
