...
const getValue = (
  ...
  const mainValue = getFromDottedKey(
    dottedKey,
    "none",
    { none: 1 },
    { ...data, lastBlock },
...
export const KeyValueKPI = ({
  ...
  const [value, updateValue] = useState(
    getValue(dottedKey, backupKey, data, lastBlock),
  );
  // if (label === "round") {
  ...
  useEffect(() => {
    ...
    if (label === "status") {
      const synced = data?.status?.data?.syncing ? "Syncing" : "Synced";
      updateValue(`${synced}`);
    }
    if (
      getDottedKeyType(dottedKey) === "lastBlock" ||
      getDottedKeyType(backupKey) === "lastBlock"
    ) {
