...
export const StakesColumn = ({
  params,
  values,
  queryData,
}: ColumnProps & {
  queryData?: { validators_dontTouch: RPCResponses<"get.pos.validators"> };
}) => {
      ...
      <Suspense
        fallback={
          <div role="status" className="max-w-sm animate-pulse">
            <div className="h-4 bg-surface-2 rounded w-full ">loading</div>
            <span className="sr-only">Loading...</span>
          </div>
        }
      >
        <span
          className={"flex flex-col space-y-2 p-2 rounded align-right  w-full"}
        >
          {values[0].value.map(
            (
              ...
              >
                <span
                  className={
                    "flex flex-row w-full justify-between space-x-2 bg-surface-1 p-2 rounded items-center"
                  }
