  ...
  if (queryData?.validators_dontTouch?.status === "success") {
    return (
      ...
      {values[0].value.map((value:{amount: string, validatorAddress: string}, index: number) => (
        <Link href={`/account/${value.validatorAddress}`} prefetch={false}><span className={"flex flex-row w-full justify-between space-x-2 bg-surface-1 p-2 rounded items-center"}
