...
import {
  ...
  accountDetails,
  accountHeader,
} from "../../assets/mockupData/accountData";
...
export default {
  ...
  args: {
    ...
    assetsData: [
      {
        name: (
          <div className={"flex gap-2 items-center"}>
            <img
              className={"h-8 w-8"}
              src={"https://cryptologos.cc/logos/lisk-lsk-logo.png?v=025"}
            />
            <div className={"flex flex-col"}>
              <Typography tag={"span"} color={"primary"}>
                {"KLY"}
              </Typography>
              <Typography tag={"span"} size={"subBody"} color={"onSurfaceLow"}>
                {"Klayr Main Token"}
              </Typography>
            </div>
          </div>
    ...
    sortOptions: ["Highest value", "Lowest value", "Alphabetical"],
  },
} as any;
...
const Template: ComponentStory<typeof Account> = (args) => {
  ...
  const totalAssets =
    args.assetsData &&
    args.assetsData.reduce((total, asset) => {
      return total + parseFloat(asset.total.replace(/[^0-9.,-]/g, ""));
    }, 0);
  ...
  return (
    <Account
      ...
      totalAssets={totalAssets ? `$${totalAssets.toString()}` : "$0"}
    />
  );
};
