...
const Template: ComponentStory<typeof Transactions> = (args) => {
  ...
  const [activeFilters, setActiveFilters] =
    useState<Array<{ filterName: string; filterValue: string }>>();
        ...
        if (
          ...
          return [
            ...
            ...prevActiveFilters.filter(
              (s) => s.filterName !== filter.filterName,
            ),
          ];
        }
  ...
  useEffect(() => {
    if (filters?.dateFilters) {
      const fromString =
        filters.dateFilters.from && filters.dateFilters.from.toLocaleString();
      const toString =
        filters.dateFilters.to && filters.dateFilters.to.toLocaleString();
    ...
    if (filters?.amountFilters) {
      const fromString =
        ...
        filters.amountFilters.from.toLocaleString();
      const toString =
        filters.amountFilters.to && filters.amountFilters.to.toLocaleString();
      const filterValue = fromString + " " + toString || "";
