    ...
    <div className={"gap-8 flex flex-col lg:flex-row"}>
      ...
      >
        ...
        <Grid flex columns={1} className={"gap-2"}>
          <Grid flex>
            <Typography className={"font-bold"} tag={"span"} color={"primary"}>
              {"Address"}
            </Typography>
      ...
      <Grid flex gap={4} className={"rounded shadow w-full lg:w-2/3 p-6 gap-4"}>
        <Grid flex columns={2} className={"justify-between"}>
          ...
          <div className={"gap-4 items-center hidden md:flex"}>
            ...
            {/*Asset Sorting*/}
            <div className={"ml-auto rounded px-1 flex gap-2"}>
              <label htmlFor={"sort"}>
                <Typography tag={"span"} color={"onSurfaceLow"}>
                  {"Sort Type"}
                </Typography>
              ...
              <select
                className={
                  "rounded bg-surface border-solid border-surface-3 border-[1px] h-8 px-2 cursor-pointer w-48"
                }
        ...
        <Grid className={"gap-2"} flex>
          ...
          {assetsData &&
            assetsData.slice(0, 3).map((row) => (
              <div
                className={
                  "flex p-4 border-solid border-[1px] border-surface-3 rounded items-center"
                }
              >
                <div style={{ flexGrow: "2" }}>{row.name}</div>
                <div
                  className={"text-right flex flex-col md:block"}
                  style={{ flexGrow: "1" }}
                >
                  <Typography tag={"span"} size={"body"}>
                    {row.amount}
          ...
          <Grid
            ...
            style={{
              maxHeight:
                showAllAssets && assetsData
                  ? `${assetsData.slice(3, assetsData.length).length * 80}px`
                  : "0px",
            }}
          >
            {assetsData &&
              assetsData.slice(3, assetsData.length).map((row) => (
                <div
                  className={
                    "flex p-4 border-solid border-[1px] border-surface-3 rounded items-center"
                  }
          ...
          <div
            className={
              "flex p-4 border-solid border-[1px] border-surface-3 rounded items-center bg-surface-2 cursor-pointer hover:bg-surface-1"
            }
